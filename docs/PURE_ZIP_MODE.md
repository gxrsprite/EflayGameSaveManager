...
| 恢复流程 | 下载 zip → 解压到配置路径 | 下载 zip → 规范化 → 保存到用户选择的位置 |
...
两个来源的 zip 在云端完全平等，按时间戳区分。PC 下载 Android 上传的 zip 后解压，Android 下载 PC 打包的 zip 后先做移动端 zip 规范化，再保存给用户。
...
Android 恢复 PC/RGSM 打包的 zip 时，需要额外处理 unit-id wrapper。普通 Folder unit 的云端 zip 可能是：
...
因此移动端 Zip restore/export 规则为：

- 下载云端 zip 到临时目录。
- 如果 zip 根目录没有文件、且只有一个子目录、且该子目录名是纯数字，则解包后去掉这层数字目录。
- 用固定的普通 ZIP deflate 重新压缩到用户选择的位置。
- 如果 zip 根目录不是单个数字目录，例如模拟器原始导出的 `SAVEDATA/...` 或 `01002AB007FD2000/...`，则直接保存原 zip。

**PC 解压恢复继续兼容；Android 保存 zip 前会做一次轻量规范化。**
...
恢复 unit 1 (Zip, Android, linked_unit_ids=[0]):
  ...
  → 没有 → 遍历 linked_unit_ids [0]
    ...
    → 下载 PC 的 zip → 因为当前 unit 1 是 Zip 类型 → 规范化后保存到用户选的位置
...
- Zip 类型 → 下载后保存到用户选择的位置；保存前若存在单个数字 unit wrapper，则临时解包、拆掉 wrapper、用普通 ZIP deflate 重压缩；Folder 类型 → 解压到目录
