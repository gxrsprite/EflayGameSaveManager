...
- `Models/CloudTransferModels.cs`
  - 云状态、备份记录、上传/下载结果。
- `Models/LocalGameBackup.cs`
  - 本地备份记录，供 UI 展示本地备份列表。
...
### 6.1 中文路径与 Unicode 编码

`System.Text.Json` 默认使用 `JavaScriptEncoder.Default`，会把非 ASCII 字符（包括中文）编码为 `\uXXXX` 转义序列。如果配置文件中包含中文游戏名或中文路径，保存后会变成不可读的 Unicode 转义：

```json
// 错误 — Default encoder
"<winLocalAppData>\\Sandfall\\Saved\\SaveGames"
"\\u4E2D\\u56FD\\u5F0F\\u76F8\\u4EB2"

// 正确 — UnsafeRelaxedJsonEscaping
"<winLocalAppData>\\Sandfall\\Saved\\SaveGames"
"\\中国式相亲"
```

**修复**：保存 `GameSaveManager.config.json` 时使用 `JavaScriptEncoder.UnsafeRelaxedJsonEscaping`：
...
`CloudSyncService` 和 `LocalSaveService` 的云端/本地清单序列化已使用此选项。仅 `GameSaveManagerConfigurationService.SaveAsync` 需要单独处理。
...
除了原始目录备份（`SaveBackupService`），Avalonia 版本增加了本地 zip 存档管理，使用与云端相同的 `Backups.json` 清单格式。
  ...
  "backups": [
    {
      ...
      "describe": "Backup all",
      "path": "E:\\save_data\\GameName\\2026-05-10_12-00-00.zip",
      "size": 12345,
      "parent": null,
      "device_id": "device-id"
    }
...
- `ListBackupsAsync(gameName, backupRoot, currentDevice)`：读取本地 `Backups.json`，返回 `LocalGameBackup` 列表。
- `CreateBackupAsync(game, backupRoot, currentDevice)`：使用 `ArchiveTransferService` 创建当前设备存档 zip，复制到存档目录，更新 `Backups.json`。
- `DeleteBackupAsync(gameName, backupRoot, date, deviceId, currentDevice)`：删除 zip 文件，从 `Backups.json` 移除记录，更新设备 head。
- `RestoreBackupAsync(game, backupRoot, currentDevice, date, deviceId)`：读取 zip 路径，调用 `ArchiveTransferService.RestoreCurrentDeviceArchive` 恢复到当前设备。

UI 展示本地备份时，设备 ID 通过 `AppSnapshot.Devices` 字典解析为设备名称显示。
...
每个存档单元进入临时 `content` 目录：

- Core/Avalonia 输出基础 RGSM v2 zip comment：`RGSM_ARCHIVE_V2` 加 `{"version":2,"compression":"deflate"}`。当前只做基础 deflate 兼容，不实现 RGSM 的 zstd preset。
- `Folder`：复制路径指向的文件夹本身到 `content/{unitId}`。若路径末尾是 `SAVEDATA`，zip 内应保留 `0/SAVEDATA/...` 这一层。
- `File`：复制文件到 `content/{unitId}/{fileName}`。
- `WinRegistry`：导出到 `content/{unitId}/registry.reg`。

所有非 Zip 存档单元都保留 unit-id wrapper：zip 内顶层是 `0/`、`1/` 等目录；其中 `Folder` 继续保留文件夹本身，例如 `0/SAVEDATA/...`。恢复端继续兼容旧的单项扁平布局和无 comment 压缩包。
...
5. 还原：
   ...
   - `Folder`：递归复制源目录到目标目录。若 zip 内带有一层与目标路径末尾同名的目录，则自动拆掉这一层；同时继续兼容旧版不带文件夹本身的扁平 zip。
   - `WinRegistry`：导入源目录下第一个 `.reg` 文件。
6. 清理临时目录。

### 9.3 移动端 Zip 恢复导出规范化

`SaveUnitType.Zip` 用于移动端的纯 Zip 模式：恢复时不是解压到固定目录，而是把下载到的云端 zip 保存到用户选择的位置，供模拟器导入。

如果云端 zip 来自 RGSM 的普通 Folder 单元打包，结构可能是：
...
移动端导出的用户可见 zip 不能保留 `0/` 这层 unit wrapper，应规范化为：
...
- 仅当 zip 根目录没有文件、且只有一个子目录、且该子目录名是纯数字时，才拆掉这层数字目录。
- 拆包和重压缩都在临时目录完成，完成后清理临时目录。
- 重压缩固定使用普通 ZIP deflate，不受未来其他压缩 preset 或压缩等级设置影响。
- 如果 zip 根目录不是单个数字目录，例如模拟器原始导出的 `SAVEDATA/...` 或 `0100.../...`，则保持原 zip 不变。
...
- 游戏详情：
  - 游戏名称、存档数量、云状态
  - 创建本地备份（zip 压缩包 + Backups.json 清单），设备 ID 显示为设备名称
  - 查看本地备份列表（设备名、大小、日期）
  - 从本地备份恢复 / 删除本地备份
  - 上传当前存档到云
  - 从云恢复当前存档
...
7. `LocalBackupService`（`SaveBackupService`）
   - 原始目录备份，按时间戳分目录。
8. `LocalSaveService`
   - 本地 zip 存档管理，使用 `Backups.json` 清单。
   - 创建（zip + 清单更新）、列表、删除、恢复。
9. `CloudStorageClient`
   - S3 兼容对象存储抽象。
10. `CloudSyncService`
    - 上传、状态、列表、下载、恢复、删除、重建 manifest。
11. `DesktopUI`
    - 游戏列表、添加游戏、路径编辑、云操作、本地存档管理。
12. `CommandLineTool`
    - 暴露核心云操作给脚本。
